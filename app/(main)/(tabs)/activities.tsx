...
const ActivitiesScreen = (props: Props) => {
  ...
  const { data, error, isFetching, isLoading } = useWasteReportActivities() as WasteReportActivitiesQuery;
  ...
  const renderItem = useCallback(({ item }: any) => {
    ...
    return (
      ...
      >
        <Avatar size="small" title={item.user.photo !== null ? undefined : "?"}>
        ...
        <View transparent style={{ flex: 1 }}>
          <Text style={{ fontWeight: "bold" }}>
            ...
            {item.post.title} <Text>by</Text> {getFullName(item.post.posted_by)}
          </Text>
        ...
        <View>
          <Image
            contentFit="cover"
            style={{ width: 50, height: 50, borderRadius: theme.spacing.md }}
